        ...
        for (var e = c[b].getElementsByTagName("*"), d = 0; d < e.length; d++)
            if (e[d].className == "video") {
                ...
                if (a.length) { a = a[0];
                    ...
                    if (h.indexOf("player.vimeo") != -1) { a.tp = "VM";
                        addVimeoEventHandlers(this, c[b], a) } else if (h.indexOf("youtube.com") != -1) { a.tp = "YT";
                        extendSliderFunForYoutube.call(this, c[b]) } } else { a = e[d].getElementsByTagName("video");
                ...
                var g = this;
                if (c[b].player) { c[b].onclick = function() { g.playVideo && g.playVideo(this.player) };
                    if (c[b].player.tp != "VA") {
                        var f = document.createElement("div");
                        f.className = "stopVideoIcon";
                        f.onclick = function(a) { g.unloadPlayer();
                            a.stopPropagation() };
                        e[d].appendChild(f) } } }
    this.playVideo = function(a, b) { this.iframe != a && this.unloadPlayer();
        if (!b) b = 0;
        ...
        switch (a.tp) {
            case "VM":
                if (this.playVideo_VM) this.playVideo_VM(a);
                    ...
                    var c = this;
                    setTimeout(function() { c.playVideo(a, ++b) }, 500) } else this.next();
                break;
            case "YT":
    ...
    this.playAutoVideo = function(a) {
        if (a.player) {
            var c = a.player.getAttribute("data-autoplay"),
                b = 0;
            if (c == "true") b = 1;
            else if (c == "1" && !a.player.AP) { b = 1;
                a.player.AP = 1 }
            if (b) { this.stop();
                this.playVideo(a.player) } } };
    this.unloadPlayer = function() {
        var a = this.iframe;
        if (a) { a.parentNode.style.zIndex = "auto";
            switch (a.tp) {
                case "VM":
...
function setSliderWhenOnPlay(a) { a.stop();
    a.iframe = a.getLis()[a.getIndex()].player;
    set_ZIndex(a.iframe, 1) }

function set_ZIndex(a, b) { a.parentNode.style.zIndex = b ? "1964" : "auto" }

function extendSliderFunForVimeo() { this.postToIframe = function(d, c, a) { this.iframe = d;
        if (window.JSON) {
            var b = { method: c };
            if (a) b.value = a;
            this.iframe.contentWindow.postMessage(JSON.stringify(b), this.iframe.getAttribute("src").split("?")[0]) } };
    this.playVideo_VM = function(a) { this.postToIframe(a, "play") };
...
function addVimeoEventHandlers(a, h, b) {!a.playVideo_VM && extendSliderFunForVimeo.call(a);
    ...
    function f() { c("addEventListener", "play");
        c("addEventListener", "finish") }

    function g() { a.iframe = b;
        setSliderWhenOnPlay(a) }

    function e() {
        if (a.iframe == b) { set_ZIndex(a.iframe, 0);
            a.backToStartFrame();
...
function onYTPlayerStateChange(a) {
    ...
    else if (a.data == 0) { setTimeout(function() { _zSlider.iframe.src = _zSlider.iframe.getAttribute("src");
            _zSlider.iframe.parentNode.parentNode.yplayer = new YT.Player(_zSlider.iframe.id, { events: { onStateChange: onYTPlayerStateChange } }) }, 500);
        _zSlider.next() } }

function onYouTubePlayerAPIReady() {
    for (var c = _zSlider.getLis(), a = 0; a < c.length; a++) {
        var b = c[a].player;
        if (b && b.tp == "YT") { b.id = _zSlider.sliderId + "-ifr" + a;
            c[a].yplayer = new YT.Player(b.id, { events: { onStateChange: onYTPlayerStateChange } }) } }
    _zSlider.playVideo_YT = function(a) { a.parentNode.parentNode.yplayer && a.parentNode.parentNode.yplayer.playVideo() } }

function extendSliderFunForhtml5VA() { this.playVideo_VA = function(a) { a.play && a.play() };
    this.unloadPlayer_VA = function(a) { a.pause && a.pause() } }

function addVAEventHandlers(c, a) {!c.playVideo_VA && extendSliderFunForhtml5VA.call(c);
    var b = { handleEvent: function(a) { a.preventManipulation && a.preventManipulation();
            switch (a.type) {
                case "click":
                    a.stopPropagation();
                    break;
                ...
                case "ended":
                    ...
                    break;
                case "loadedmetadata":
                    this.loaded() } }, play: function() { setSliderWhenOnPlay(c) }, ended: function() { set_ZIndex(a, 0);
            c.next();
            a.currentTime = 0 }, loaded: function() {} };
    addEvent(a, "click", b);
    addEvent(a, "play", b);
    addEvent(a, "ended", b);
